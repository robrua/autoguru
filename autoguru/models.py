...
@pre_save(Answer)
async def signal_pre_save(sender, instance: Answer, using_db, update_fields) -> None:
...
class Question(Model):
    ...
    text = fields.TextField(default="")
    formatted_text = fields.TextField(default="")
    answer: fields.ForeignKeyRelation[Answer] = fields.ForeignKeyField("models.Answer", related_name='questions')
...
@pre_save(Question)
async def signal_pre_save(sender, instance: Question, using_db, update_fields) -> None:
    if not instance.formatted_text:
        instance.formatted_text = instance.text
